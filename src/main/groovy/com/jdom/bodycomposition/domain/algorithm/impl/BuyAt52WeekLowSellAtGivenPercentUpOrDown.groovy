 ...
 */
class BuyAt52WeekLowSellAtGivenPercentUpOrDown implements Algorithm {
    ...
    void actionsForDay(Broker broker, final PortfolioValue portfolioValue, final List<DailySecurityData> dayEntries,
        ...
        if (positions.size() < MAX_NUMBER_OF_POSITIONS_TO_HOLD) {
            ...
            Set<Long> fiftyTwoWeekLowIds = new HashSet<>(dailyMetrics.collect { it.fiftyTwoWeekLow.id })
            Set<Long> todayIds = new HashSet<>(dayEntries.collect { it.id })

            def intersection = todayIds.intersect(fiftyTwoWeekLowIds)

            log.info "Found ${intersection.size()} 52 week lows for today."

            def dayEntriesToConsiderBuying = dayEntries.findAll{ intersection.contains(it.id) }

            boolean submittedOrder = false
            int orderAttempt = 0;
            while (!submittedOrder && orderAttempt < MAXIMUM_ORDER_SEARCHES_TO_TRY_PER_DAY) {

                DailySecurityData entry = dayEntriesToConsiderBuying.get(random.nextInt(dayEntriesToConsiderBuying.size()))
                if (entry.high < toMoney('$5')) { // Skip really low valued stocks
                    continue
                }
                if ((entry.close == entry.low) && (entry.volume > EXCLUSIVE_LOW_VOLUME_REQUIRED)) {
                    // Find an entry that closed at its low
