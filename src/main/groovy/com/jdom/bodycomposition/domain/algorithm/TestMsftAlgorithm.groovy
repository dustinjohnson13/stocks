...
class TestMsftAlgorithm implements Algorithm {
    ...
    @Override
    boolean includeTicker(final YahooStockTicker ticker) {
        return ticker.ticker == 'MSFT'
    }

    @Override
    List<PortfolioTransaction> actionsForDay(Portfolio portfolio, final YahooStockTickerData dayEntry) {
...
//        (5,'2010-07-15',2550,2551,2559,2498,56934700,2257),
            return [new BuyTransaction(dayEntry.ticker, dayEntry.date, 2, 2550, portfolio.commissionCost)]
...
//            (5,'2011-09-30',2520,2489,2550,2488,54060500,2270),
            return [new SellTransaction(dayEntry.ticker, dayEntry.date, 2, 2489, portfolio.commissionCost)]
        }
        return []
    }
}
